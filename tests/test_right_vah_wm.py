...
def start():
    ...
    for _U in range(START_U_FORM, MAX_U_FORM+STEP_U_FORM, STEP_U_FORM):
        ...
        if _U == START_U_FORM:
            ...
            dt.calc_wom()
            massiv_field = df.calc_wom(massiv_potential=d.massiv_potential)
        ...
        massiv_field = df.calc(massiv_potential=d.massiv_potential, massive_for_check_vacancies=probgen.massive_for_check_vacancies)
    ...
    # # Рекомбинация
    # for _U in range(MAX_U_FORM, 0, -STEP_U_FORM):
    #     print('U:', _U / 10)
